...
for i in range(len(n_cells)):  # for each dataset
    ...
    for j in range(10):  # for each iteration
        ...
        savename = prefix + "_paths_SEMITONES_UMAP_cosine.txt"
        escores.to_csv(savename, sep="\t")

        # save the permutation scores
        savename = prefix + "_paths_SEMITONES_UMAP_cosine_pscores.txt"
        pscores.to_csv(savename, sep="\t")

# save the times for enrichment scoring
pd.DataFrame(times).to_csv("paths_SEMITONES_UMAP_cosine_times.txt", sep="\t")
# save the permutation enrichment scores
pd.DataFrame(times_with_p).to_csv("paths_SEMITONES_UMAP_cosine_timesp.txt",
                                  sep="\t")
