...
for i in range(len(n_cells)):  # for each dataset
    ...
    for j in range(10):  # for each iteration
        ...
        savename = prefix + "_groups_SEMITONES_UMAP_rbf.txt"
        escores.to_csv(savename, sep="\t")

        # save the permutation scores
        savename = prefix + "_groups_SEMITONES_UMAP_rbf_pscores.txt"
        pscores.to_csv(savename, sep="\t")

# save the times for enrichment scoring
pd.DataFrame(times).to_csv("groups_SEMITONES_UMAP_rbf_times.txt", sep="\t")
# save the permutation enrichment scores
pd.DataFrame(times_with_p).to_csv("groups_SEMITONES_UMAP_rbf_timesp.txt",
                                  sep="\t")